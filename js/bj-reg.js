// A function to get the user's account name
function getuser() {
...
// function to update balance on site
...
// function to update users account balance
...
var bj = {
    ...
    bankroll : 1000000,
    //user : getuser(),
    min_bet : 25, // bet must be at least
	  max_bet : 5000, // bet can't be above table max
    bet_size : 25, //actual amount bet
    multi : 5, //increase bet multiplier 
    betup : 0,
    ...
    betup : () => {
		...
		if (bj.bet_size > bj.max_bet) {
            bj.bet_size = bj.max_bet; //if bet is more than player has, change to all in
        }
        else {
    ...
    check : () => {
      ...
      if (bj.player.length==2 && bj.dealer.length==2) {
        ...
        if (winner==null && bj.ppoints==21) {
          ...
          updateBalance((bet_size + (3/2 * bj.bet_size)));
          getBalance();
        }
        // DEALER WINS
